...
impl Database {
    ...
    pub async fn migrate(&self) -> Result<(), sqlx::Error> {
        ...
        // Create tables manually using the helper function
...
pub async fn create_tables(pool: &PgPool) -> Result<(), sqlx::Error> {
    ...
    // Create indexes
    sqlx::query(r#"CREATE INDEX IF NOT EXISTS idx_books_title ON books USING gin(to_tsvector('english', title))"#).execute(pool).await?;
    sqlx::query(r#"CREATE INDEX IF NOT EXISTS idx_books_authors ON books USING gin(authors)"#).execute(pool).await?;
    sqlx::query(r#"CREATE INDEX IF NOT EXISTS idx_book_copies_status ON book_copies(status)"#).execute(pool).await?;
    sqlx::query(r#"CREATE INDEX IF NOT EXISTS idx_transactions_user_id ON transactions(user_id)"#).execute(pool).await?;
    sqlx::query(r#"CREATE INDEX IF NOT EXISTS idx_transactions_due_date ON transactions(due_date)"#).execute(pool).await?;
    sqlx::query(r#"CREATE INDEX IF NOT EXISTS idx_reservations_user_book ON reservations(user_id, book_id)"#).execute(pool).await?;
